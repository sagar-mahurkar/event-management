...
export class EventService {
    ...
    async searchEvents(query: any) {
        ...
        // Need bookings to compute availability
        const events = await eventRepo.find({
            where,
            skip,
            take,
            relations: ["creator", "bookings"],
            order: { dateTime: "ASC" }
        });
        ...
        if (availableOnly) {
            return events.filter(event => {
                ...
                return event.capacity - bookedCount > 0;
            });
        }
    ...
    async getOrganizerEvents(organizerId: number) {
        return eventRepo.find({
            where: { createdBy: organizerId }, // createdBy = number column
            relations: ["creator", "bookings", "bookings.user"],
            order: { dateTime: "ASC" }
        });
    }
    ...
    async createEvent(eventData: any, userId: number) {
        ...
        const newEvent = eventRepo.create({
            ...
            createdBy: user.id, // foreign key column
        });
